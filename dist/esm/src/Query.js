...
export default class Query {
    ...
    create(data = {}) {
        if (Resolver.db().events.creating)
            Resolver.db().events.creating();
    ...
    update(id, data = {}) {
        if (Resolver.db().events.updating)
            Resolver.db().events.updating();
        ...
        this.addIndexesByItem(item);
        oldItem = null;
        if (Resolver.db().events.updated)
            Resolver.db().events.updated();
    ...
    delete(id) {
        ...
        if (Resolver.db().events.deleting)
            Resolver.db().events.deleting();
        let item = this.getItem(id);
        if (!item)
            return;
        ...
        this.saveTableData(tableData);
        if (Resolver.db().events.deleted)
            Resolver.db().events.deleted();
