...
class Query {
    ...
    create(data = {}) {
        if (Resolver_js_1.default.db().events.creating)
            Resolver_js_1.default.db().events.creating();
        let tableData = this.getTableData(), id = ++tableData.lastPrimaryKey, item = null;
        ...
        if (Resolver_js_1.default.db().events.creating)
            Resolver_js_1.default.db().events.created();
        return item;
    }
    get() {
        let data = [], filteredIndex = this.getFilteredIndex();
    ...
    findLatest() {
        let tableData = this.getTableData(), primaryKeys = Object.keys(tableData.index), latestPrimaryKey = Math.max(...primaryKeys);
        return this.find(latestPrimaryKey);
    }
    find(id = null) {
        if (!id)
            throw new Error('Please specify an identifier to find a row');
    ...
    update(id, data = {}) {
        if (Resolver_js_1.default.db().events.updating)
            Resolver_js_1.default.db().events.updating();
        ...
        this.addIndexesByItem(item);
        oldItem = null;
        if (Resolver_js_1.default.db().events.updated)
            Resolver_js_1.default.db().events.updated();
        Resolver_js_1.default.db().executeOnUpdateCallbackForTable(this.tableKey(), item);
        return true;
    }
    delete(id) {
    ...
    addToDeletingBuffer(id) {
        return Resolver_js_1.default.db().addToDeletingBuffer(this.tableKey(), id);
    }
    removeFromDeletingBuffer(id) {
        return Resolver_js_1.default.db().removeFromDeletingBuffer(this.tableKey(), id);
    }
    ...
    getItemData(id) {
        let itemKey = this.tableItemKey(id), itemData = this.dbDriver().get(itemKey);
        if (!itemData)
            return null;
        return itemData;
    }
    saveItem(id, data) {
        ...
        if (this.model.timestamps()) {
            if (!data.createdAt)
                data.createdAt = (0, moment_1.default)().format('YYYY-MM-DD HH:mm:ss');
            data.updatedAt = (0, moment_1.default)().format('YYYY-MM-DD HH:mm:ss');
        }
    ...
    getTableData() {
        let tableKey = this.tableKey(), tableData = this.dbDriver().get(tableKey);
        if (!tableData)
    ...
    checkForeignKeyConstraints(item) {
        ...
        if (hasManyItemsCount)
            throw new Error('Cannot delete a parent item: a foreign key constraint fails');
    }
    ...
    removeIndexesByItem(item) {
        item.belongsToRelationships().forEach(belongsToRelationship => this.removeItemFromParentHasManyIndex(belongsToRelationship, item));
    }
    ...
    manipulateHasManyIndex(manipulationCallback, relationship, item) {
        ...
        if (!parent)
            return;
        let parentQuery = parent.constructor.getQuery(), parentIndex = parentQuery.getItemIndex(parent);
        if (!parentIndex)
            return;
    ...
    getItemHasManyIndex(item, relationship) {
        let itemIndex = item.constructor.getQuery().getItemIndex(item), indexKey = `${item.getTable()}.${relationship.foreignKey}`;
    ...
    tableStructure() {
        return {
            ...
            items: [],
            relations: [],
            reladbVersion: package_json_1.default.version,
        };
    }
    indexStructure() {
    ...
    dbDriver() {
        return Resolver_js_1.default.db().driver.setTable(this.model.table());
    }
    ...
    log() {
        if (Resolver_js_1.default.db().mode === 'development') {
            if (!this.lastLogTime)
                this.lastLogTime = (0, moment_1.default)();
            let difference = (0, moment_1.default)().diff(this.lastLogTime);
