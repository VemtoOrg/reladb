...
const ImporterModel_js_1 = __importDefault(require("./ImporterModel.js"));
class Importer {
    ...
    importData() {
        this.importerModel = ImporterModel_js_1.default;
        this.importingData.exportedItems.forEach(itemIdentifier => {
            let identifierSections = itemIdentifier.split(':'), table = identifierSections[0], id = identifierSections[1];
            this.importItem(id, table);
        });
        this.finish();
    }
    importItem(id, table) {
        let itemData = this.importingData.tables[table][`item_${id}`], itemIdentifier = `${table}:${id}`;
    ...
    importHasManyRelationshipsItems(originalItemId, originalItemTable, importedItem) {
        ...
        if (!originalTableData.index[originalItemId] || !originalTableData.index[originalItemId].hasMany)
            return;
        Object.keys(originalTableData.index[originalItemId].hasMany).forEach(indexName => {
            let indexItems = originalTableData.index[originalItemId].hasMany[indexName], indexSections = indexName.split('.'), relationshipTable = indexSections[0], foreignName = indexSections[1];
            indexItems.forEach(originalRelatedItemId => {
                ...
                new Query_js_1.default(this.importerModel).saveTableData(tableData);
            });
        });
    }
    getTableDataWithImportedItemIndex(originalItemTable, importedItem, indexName) {
        this.importerModel.table = () => originalItemTable;
        let tableData = (new Query_js_1.default(this.importerModel)).getTableData();
        if (!tableData.index[importedItem.id]) {
            tableData.index[importedItem.id] = Query_js_1.default.basicIndexStructure();
        }
