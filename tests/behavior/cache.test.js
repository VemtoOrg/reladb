...
test("it can cache items with recursive relations", () => {
    ...
    let firstRelationship = Relationship.create({ name: "test", entityId: userEntity.id }),
        secondRelationship = Relationship.create({
            name: "test",
            entityId: userEntity.id,
            inverseId: firstRelationship.id,
        })
...
test("it removes unnecessary commands", () => {
    ...
    let commandsEditingUser = Resolver.db().commands.filter((command) => command.command == "set item_1 on users")

    expect(commandsEditingUser.length).toBe(1)

    let commandsEditingPost = Resolver.db().commands.filter((command) => command.command == "set item_1 on posts")
